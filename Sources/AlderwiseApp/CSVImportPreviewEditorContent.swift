...
struct CSVImportPreviewEditorContent: View {
    ...
    private var mappingSummary: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            if Self.descriptionSemanticsLines(for: preview).isEmpty == false {
                VStack(alignment: .leading, spacing: 6) {
                    ForEach(Self.descriptionSemanticsLines(for: preview), id: \.self) { line in
                        Text(line)
                            .font(.subheadline)
...
extension CSVImportPreviewEditorContent {
    nonisolated static func descriptionSemanticsLines(for preview: CSVImportPreview) -> [String] {
        var lines: [String] = []

        if let guidance = preview.profile.previewGuidanceText {
